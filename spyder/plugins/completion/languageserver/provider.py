...
class LanguageServerProvider(BaseCompletionProvider):
    ID = 'lsp'
    ...
    def __init__(self, parent):
        ...
        self.sig_exception_occurred.connect(
            self._main.console.handle_exception)
    ...
    def get_languages(self):
        ...
        """
        languages = ['python']
        all_options = CONF.options('lsp-server')
    ...
    def get_root_path(self, language):
        ...
        # FIXME:
        if self._main and self._main.projects:
            path = self._main.projects.get_active_project_path()
    ...
    def register_client_instance(self, instance):
        ...
        if self._main:
            main = self._main
            main.sig_pythonpath_changed.connect(self.update_python_path)
            ...
            if main.editor:
                instance.sig_initialize.connect(
                    main.editor.register_completion_capabilities)
                main.editor.sig_editor_focus_changed.connect(
            ...
            if main.projects:
                instance.sig_initialize.connect(
                    main.projects.register_lsp_server_settings)
    ...
    def receive_response(self, response_type, response, language, req_id):
        if req_id in self.requests:
            ...
            self.sig_response_ready.emit(self.ID, req_id, response)
